...
class CDR_Data{
    ...
    private function q_done($q_res_data){
        ...
        }else if('10000666' == $this->exten || '10000777' == $this->exten ){
            $res_data = json_decode($q_res_data, true);
            $this->app_10000666_777($res_data);
        }else if('10000109' == $this->exten){
            $res = Models\Extensions::findFirst("number='{$this->vars['number']}'");
            $context = ($res != null)?'all_peers':'';
            $this->UserEvent("GetContest,chan1c:{$this->vars['tehnology']}/{$this->vars['number']},peercontext:{$context}");
        }else if('10000111' == $this->exten){
            $stat = file_get_contents('/var/etc/http_auth');
            $this->UserEvent("AsteriskSettings,chan1c:{$this->vars['chan']},Statistic:{$stat},DefaultContext:all_peers,DialplanVer:1.0.0.6,usemp3player:1,autoanswernumber:*8");
            $this->GetHints();
        }else if('10000222' == $this->exten){
            $this->app_10000222();
        }
    ...
    private function exec_query(){
        ...
        }else if('10000111' == $this->exten || '10000109' == $this->exten || '10000222' == $this->exten){
            $this->q_done(null);
            return;
        }else{
            return;
        ...
        if($message == false){
            Util::sys_log_msg('miko_ajam', "Error get data from queue 'select_cdr'. ");
            $this->q_done("[]");
        }else{
            $result_data = "[]";
    ...
    private function app_10000222(){
        ...
        if(!in_array($this->vars['dbFamily'], $commands)){
            ...
            if($ret['result']==1&&$ret['code']==200){
                ...
                $this->UserEvent("DB_{$this->vars['dbFamily']},chan1c:{$this->vars['chan']},key:{$this->vars['key']},val:{$ret['data']}");
            }else{
                // Не установлена!
                $this->UserEvent("DB_{$this->vars['dbFamily']},chan1c:{$this->vars['chan']},key:{$this->vars['key']},val:");
            }
            ...
            if($this->vars['val']==''){
                ...
                // Установка статуса
                $ret = $this->agi->evaluate("DATABASE PUT {$this->vars['dbFamily']} {$this->vars['key']} {$this->vars['val']}");
            }
            if($ret['result']==1&&$ret['code']==200){
                // Успех выполнения операции
                $this->UserEvent("DB_{$this->vars['dbFamily']},chan1c:{$this->vars['chan']},key:{$this->vars['key']},val:{$this->vars['val']}");
            }else{
                // Были ошибки
                $this->UserEvent("Error_data_put_{$this->vars['dbFamily']},chan1c:{$this->vars['chan']},key:{$this->vars['key']},val:{$this->vars['val']}");
            }
    ...
    private function app_10000555($res_data){
        $fields = [ 'start','src_num','dst_num','src_chan',
            'dst_chan','billsec','disposition','UNIQUEID','recordingfile',
            'v1','appname','linkedid','did'];
        $result = ""; $ch = 0;
    ...
    private function app_10000666_777($res_data){
        ...
        foreach ($res_data as $ar_str){
            if(Util::rec_file_exists($ar_str['recordingfile'])){
    ...
    private function old_10000666_777($id){
        if(!file_exists('/storage/usbdisk1/mikopbx/astlogs/asterisk/askozia_http_settings.php')) return '';
        ...
        $output   = array();
        $cdr_db = dirname(Cdr::getPathToDB()).'/master.db';
...
try{
    ...
    Util::sys_log_msg('miko_ajam', $e->getMessage());
}