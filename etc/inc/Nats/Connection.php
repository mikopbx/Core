...
class Connection
{
     ...
     * @param $line
     */
    private function handleERR($msg){
        if(!key_exists("ERROR.MSG", $this->subscriptions)){
            return;
        }
        $func = $this->subscriptions['ERROR.MSG'];
        $msg = trim(str_replace('Permissions Violation','', $msg));
        if(is_array($func)){
            call_user_func($func, $msg);
        } else if (is_callable($func) === true) {
            $func($msg);
     ...
     * @param string   $payload  Message data.
     * @param \Closure | array $callback Closure to be executed as callback.
     * @throws Exception
     */
    ...
    public function setErrorHendler($callback){
    ...
    public function wait($quantity = 0)
    {
        ...
        while (is_resource($this->streamSocket) === true && feof($this->streamSocket) === false && empty($info['timed_out']) === true) {
            ...
            if ($this->isErrorResponse($line) === true) {
                $line = trim($line);
