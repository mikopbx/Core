...
class p_IAX extends ConfigClass {
    ...
    private function get_providers(){
        ...
        $this->data_providers   = [];
        $db_data = \Models\Iax::find("disabled IS NULL OR disabled = '0'");
        foreach ($db_data as $peer) {
            /** @var \Models\Iax $peer */
            $arr_data = $peer->toArray();

            // $network_filter = Models\NetworkFilters::findFirst($peer->networkfilterid);
            ...
            $codecs = \Models\IaxCodecs::find($filter);
    ...
    protected function generateConfigProtected($general_settings){
        ...
        Util::file_write_content($this->astConfDir."/iax.conf", $conf);
     ...
     */
    static function iax_reload(){
        global $g;
        $result = array(
            'result'  => 'ERROR'
        );
        $config = new Config();
        $arr_gs = $config->get_general_settings();
        $iax    = new p_IAX($g);
