...
class PBX {
     ...
     * @return array
     */
	static function init_modules($g){
	    $arrObject = [];
        $arr = array(
            'p_ExternalPhones',
            'p_OtherConfigs',
     ...
     * @return array
     */
	static function get_asterisk_dirs(){
		global $g;
		$path2dirs = [];
		...
		if( \Storage::is_storage_disk_mounted() ){
			...
			$g['pt1c_cdr_db_path'] 	     = "/var/asterisk/log/cdr.db";
		}
     ...
     * @param bool $booting
     */
	static function start($booting = false) {
     ...
     */
	public function dialplan_reload(){
        global $g;
        $result = array(
            'result'  => 'ERROR'
        );
     ...
     */
	private function modules_conf_generate(){
		$conf = "[modules]\n".
				"autoload=no\n";
		$modules = array();
     ...
     */
	private function indication_conf_generate($country = "ru"){
		global $g;
     ...
     */
	private function features_generate(){
     ...
     */
	static function features_reload(){
        $pbx = new PBX();
     ...
     * @return array
     */
    static function manager_reload(){
        $pbx = new PBX();
    ...
    static function modules_reload(){
        $pbx = new PBX();
     ...
     */
	private function manager_conf_generate(){
	    $conf = "[general]\n".
				"enabled = yes\n".
     ...
     */
    private function http_conf_generate(){
     ...
     * Обновление системы.
     */
    static function update_system_config(){
        $g =  $GLOBALS['g'];
        $needreboot = false;
        ...
        if (!$connection->tableExists('m_PbxExtensionRelationship')) {
            $columns = [
                new \Phalcon\Db\Column('id',                $type_key),
                new \Phalcon\Db\Column('model',             $type_str),
                new \Phalcon\Db\Column('relationship',      $type_str),
                new \Phalcon\Db\Column('fields',            $type_str),
                new \Phalcon\Db\Column('referenceModel',    $type_str),
                new \Phalcon\Db\Column('referencedFields',  $type_str),
                new \Phalcon\Db\Column('alias',             $type_str),
                new \Phalcon\Db\Column('allowNulls',        $type_int),
                new \Phalcon\Db\Column('action',            $type_int),
                new \Phalcon\Db\Column('moduleUniqid',      $type_str),
            ];
            $result = $connection->createTable( 'm_PbxExtensionRelationship', null, ['columns' => $columns ]);
            if($result == true){
                // Необходима перезагрузка.
                $needreboot = true;
                echo " - Create new table m_PbxExtensionRelationship.\n";
            }else{
                Util::sys_log_msg('update_system_config', 'Error: Failed to create table m_PbxExtensionRelationship.');
            }
        }
