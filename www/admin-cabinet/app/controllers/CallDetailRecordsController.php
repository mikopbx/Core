...
class CallDetailRecordsController extends BaseController {
	 ...
	 */
	private function prepareConditionsForSearchPhrases(&$searchPhrase, &$parameters){
			...
			if(preg_match_all("/\d{2}\/\d{2}\/\d{4}/", $searchPhrase, $matches))
			{
				if (count($matches[0])==1){
					...
					$parameters['bind']["dateFromPhrase2"]=$tomorrowDate;
					$searchPhrase = str_replace($matches[0][0], "", $searchPhrase);
					$searchPhrase = str_replace($matches[0][1], "", $searchPhrase);
				}
			}
			// Поищем линкдиды
			...
			$searchPhrase = str_replace(array( '(', ')','-','+' ), '', $searchPhrase);
			...
			if(preg_match_all("/\d+/", $searchPhrase, $matches)){
				...
				if (count($matches[0])==1){
					...
					if ($extensionsLength == strlen($matches[0][0]) AND $extensionsLength == strlen($matches[0][1])){
					...
					} else if ($extensionsLength == strlen($matches[0][0]) AND $extensionsLength != strlen($matches[0][1])){
				...
				if (count($matches[0])==1 OR count($matches[0])==2){
