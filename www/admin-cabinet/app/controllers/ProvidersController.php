...
class ProvidersController extends BaseController
{
	 ...
	 */
	public function modifysipAction($uniqid=null){
	 ...
	 * @param string $uniqid Уникальный идентификатор провайдера, если мы открываем существующего
	 */
    public function modifyiaxAction($uniqid=null){
        // Списоок доступных кодеков
        $codecs=array();
        $availableCodecs = Codecs::find();
        foreach ($availableCodecs as $codec){
	 ...
	 * @param string $uniqid Уникальный идентификатор провайдера, если мы открываем существующего
	 */
	public function enableAction($type, $uniqid=null){
	 ...
	 * @param string $uniqid Уникальный идентификатор провайдера, если мы открываем существующего
	 */
    public function disableAction($type, $uniqid=null){
        $this->view->success = false;
        switch($type) {
            case "iax":
                {
	 ...
	 */
    private function saveProvider($data, $type){
	 ...
	 * @return bool результат сохранения
	 */
	private function saveCodecs($data, $type){
		$availableCodecs = Codecs::find();
		if ($type=='iax') {
			$classCodecs = 'Models\IaxCodecs';
			$fieldUid = 'iaxuid';
		} else { //sip
			$classCodecs = 'Models\SipCodecs';
			$fieldUid = 'sipuid';
		}
		...
		foreach ($availableCodecs as $codec) {
			...
			if (array_key_exists('codec_' . $key, $data) && $data['codec_' . $key]=='on') {
				...
				if (!$newCodec) $newCodec = new $classCodecs();
