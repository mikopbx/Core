...
class TestOutboundRoutes:
    ...
    def test_03_get_by_id(self, api_client, sample_outbound_route):
        response = api_client.get(f'outbound-routes/{sample_outbound_route}')
        assert_api_success(response, "Failed to get record")
        print(f"✓ Retrieved outbound route: {sample_outbound_route}")
