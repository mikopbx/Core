...
- [ ] PJSUA library compiled with Python SWIG bindings
- [ ] Python wrapper module created for easy import
- [ ] Can register SIP endpoints programmatically from Python
- [ ] Can make outbound calls with proper CDR creation
- [ ] Can answer inbound calls
- [ ] Can act as both peer (internal extension) and trunk (external provider)
- [ ] Integration tests demonstrate all capabilities
- [ ] Documentation for using PJSUA in MikoPBX tests
...
- Current tool: GoPhone (binary in `tests/pycalltests/bin/darwin-arm64/gophone`)
- Helper module: `gophone_helper.py`
- Test extensions: 201, 202, 203 (credentials in `tests/api/fixtures/employee.json`)

### Known GoPhone Issues
1. CDR records not created for GoPhone calls
2. Recording files use format `mikopbx-{UNIQUEID}_{HASH}.wav` instead of expected `*-{src}-{dst}-*.wav`
3. Calls may not properly route to real endpoints
4. Limited DTMF and codec negotiation support

### Requirements
- Must work on macOS (darwin-arm64)
- Must integrate with existing pytest test suite
- Should support same interface as GoPhoneManager for easy migration
- Must create proper CDR records
- Must generate recordings with standard naming

### PJSUA Features Needed
- SIP REGISTER
- SIP INVITE (outbound calls)
- SIP answer (inbound calls)
- DTMF RFC2833
- Multiple codecs (alaw, ulaw, g729, g722, opus)
- Media handling (audio playback/recording)
- Call transfer (blind and attended)

## Work Log
- [2025-11-15] Task created - need to replace GoPhone with PJSUA for proper SIP testing
