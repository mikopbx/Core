...
## Success Criteria
- [x] Файл конфигурации с User-Agent'ами записывается через Util::fileWriteContent
- [x] Пользователь может редактировать список через "Кастомизация системных файлов"
- [x] Пакеты с User-Agent из списка дропаются на iptables (UDP+TCP, SIP+TLS порты)
- [x] Правила добавляются при каждом firewall reload (45 правил = 15 агентов × 3 порта)
- [x] Легитимные SIP-клиенты не блокируются (regex sanitize + known list only)
- [x] Комментарии (#) и пустые строки в конфиге игнорируются
