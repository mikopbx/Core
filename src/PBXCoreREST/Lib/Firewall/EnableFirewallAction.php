 ...
 * Class EnableAction
 * 
 * Enables the firewall and fail2ban services.
 *
 * @package MikoPBX\PBXCoreREST\Lib\Firewall
 */
class EnableFirewallAction
{
    /**
     * Enable firewall and fail2ban
     *
     * @return PBXApiResult The result of the enable operation
     */
        ...
        try {
            ...
            if (!$fail2BanEnabled->save()) {
                ...
                foreach ($errors as $error) {
                    $res->messages['error'][] = "Failed to enable Fail2Ban: " . $error->getMessage();
                }
                $res->success = false;
                return $res;
            }
            ...
            if (!$firewallEnabled->save()) {
                ...
                foreach ($errors as $error) {
                    $res->messages['error'][] = "Failed to enable Firewall: " . $error->getMessage();
                }
                $res->success = false;
                return $res;
            }
            ...
            $res->messages['info'][] = 'Firewall and Fail2Ban have been enabled';
            $res->success = true;
        } catch (\Exception $e) {
