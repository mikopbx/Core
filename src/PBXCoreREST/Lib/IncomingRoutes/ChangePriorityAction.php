...
/**
 * Action for changing priority of incoming routes
 * 
 * @api {post} /pbxcore/api/v2/incoming-routes/changePriority Change incoming routes priority
 * @apiVersion 2.0.0
...
class ChangePriorityAction
{
    ...
    {
        // Skip default route (ID=1)
        if ((int)$routeId === 1) {
            return ['success' => false, 'error' => null]; // Skip silently
        }
        
        $route = IncomingRoutingTable::findFirstById($routeId);
