...
class CallerIdDidProcessor
{
    ...
    private function generateCallerIdFromCustomHeader(): string
    {
        ...
        if (!empty($this->providerSettings['cid_parser_regex'])) {
            ...
            // a substring, so the extraction is delegated to the extract_did_cid.php AGI,
            // which applies PCRE and writes the result into fromCid. The pattern is
            // base64-encoded so dialplan-special characters in it survive the parser.
            $encodedRegex = base64_encode($this->providerSettings['cid_parser_regex']);
    ...
    private function generateDidFromTo(): string
    {
        ...
        // If extracted DID differs from current EXTEN, redirect call. The FILTER() guard
        // ensures the (attacker-influenced) value contains only inbound-extension characters
        // before it is interpolated into Goto, so a stray comma/newline cannot split the
        // Goto arguments and reroute the call.
        $dialplan .= "\tsame => n,ExecIf(\$[\"x\${toNum}\" != \"x\" && \"\${toNum}\" != \"\${EXTEN}\" && \"\${FILTER(0-9a-zA-Z*#+,\${toNum})}\" == \"\${toNum}\"]?Goto({$this->providerId}-incoming,\${toNum},1))\n";
    ...
    private function generateDidFromDiversion(): string
    {
        ...
        // before it is interpolated into Goto, so a stray comma/newline cannot split the
        // Goto arguments and reroute the call.
        $dialplan .= "\tsame => n,ExecIf(\$[\"x\${divNum}\" != \"x\" && \"\${divNum}\" != \"\${EXTEN}\" && \"\${FILTER(0-9a-zA-Z*#+,\${divNum})}\" == \"\${divNum}\"]?Goto({$this->providerId}-incoming,\${divNum},1))\n";
    ...
    private function generateDidFromCustomHeader(): string
    {
        ...
        if (!empty($this->providerSettings['did_parser_regex'])) {
            // Use regex parser. Asterisk's REGEX() only tests for a match and cannot extract
            // a substring, so the extraction is delegated to the extract_did_cid.php AGI,
            // which applies PCRE and writes the result into didNum. The pattern is
            // base64-encoded so dialplan-special characters in it (comma in {10,11}, ';',
            // quotes) survive the extensions.conf parser.
            $encodedRegex = base64_encode($this->providerSettings['did_parser_regex']);
