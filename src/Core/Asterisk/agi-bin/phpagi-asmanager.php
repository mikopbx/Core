...
class AGI_AsteriskManager {
     ...
     * @return array of parameters
     */
    function sendRequestTimeout($action, $parameters = array()){
        $req = "Action: $action\r\n";
        foreach ($parameters as $var => $val) $req .= "$var: $val\r\n";
        $req .= "\r\n";
        if(!is_resource($this->socket)){
     ...
     * @return array|bool
     */
    function pingAMIListner($pingname = 'CdrConnector'){
     ...
     */
    function disconnect(){
        if ($this->_loggedIn == TRUE) $this->logoff();
        if(is_resource($this->socket)){
            fclose($this->socket);
        }
    }
