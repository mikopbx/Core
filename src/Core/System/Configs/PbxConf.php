...
class PbxConf extends SystemConfigClass
{
    ...
    public function generateMonitConf(): bool
    {
        ...
        $safeStartScript = '/usr/sbin/safe_asterisk_start';

        // Generate guard script that prevents duplicate instances
        $this->generateSafeStartScript($safeStartScript, $binPath);
    ...
    /**
     * Generates a guard shell script that prevents monit from starting a duplicate
     * Asterisk instance when Asterisk re-execs itself via 'core restart now'.
     *
     * @param string $scriptPath Path where the script will be written.
     * @param string $binPath Path to the asterisk binary.
     */
    private function generateSafeStartScript(string $scriptPath, string $binPath): void
    {
        $pidFile = $this->runDirPath . '/' . self::PROC_NAME . '.pid';
        $script = '#!/bin/sh' . PHP_EOL
            . '# Guard against duplicate Asterisk instances' . PHP_EOL
            . '# Generated by MikoPBX PbxConf' . PHP_EOL
            . 'PIDFILE="' . $pidFile . '"' . PHP_EOL
            . 'if [ -f "$PIDFILE" ]; then' . PHP_EOL
            . '    PID=$(cat "$PIDFILE" 2>/dev/null)' . PHP_EOL
            . '    if [ -n "$PID" ] && kill -0 "$PID" 2>/dev/null; then' . PHP_EOL
            . '        logger -t safe_asterisk_start "Asterisk already running (PID $PID), skip"' . PHP_EOL
            . '        exit 0' . PHP_EOL
            . '    fi' . PHP_EOL
            . 'fi' . PHP_EOL
            . 'rm -f "$PIDFILE"' . PHP_EOL
            . 'exec ' . $binPath . ' -F' . PHP_EOL;
